...
// We use a TRANSACTION to ensure that if anything fails
// (e.g. order saves but items don't), everything rolls back.
// Either the complete order is saved, or nothing is saved.
if ($action === 'create') {
    ...
    // -- Items: sent as JSON string from JS
    ...
    try {
        ...
        // We fetch the current price from the DB as the snapshot.
        // This prevents a crafted POST from passing a fake price.
...
// ── UPDATE ORDER STATUS ──────────────────────────────────────
// Business rule: Pending → In Progress → Completed only.
// When marked Completed, a sales record is auto-created.
